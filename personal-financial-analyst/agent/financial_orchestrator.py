...
async def _auto_approve_all(
    ...
    Returns:
        PermissionResultAllow to grant access
    """
...
def _save_json(
    ...
    Args:
        ...
        filename: Target filename under raw_data directory
    """
...
def _detect_subscriptions(
    ...
    Filters all transactions marked as recurring=True with a negative amount
    (outgoing money), treating each as a monthly subscription.
    ...
    for tx in bank_transactions + credit_card_transactions:
        if tx.get("recurring") is True and float(tx.get("amount", 0)) < 0:
            # Bank transactions use 'description'; credit card use 'merchant'
            name = tx.get("description") or tx.get("merchant", "Unknown")
...
async def _fetch_financial_data(
    ...
    Configures MCP server connections to the Bank Account Server (port 5001)
    and Credit Card Server (port 5002), calls the respective fetch tools,
    and persists the raw responses to disk.
    ...
    # Keys MUST match the FastMCP server name strings exactly
    mcp_servers = {
        "Bank Account Server": {
            "type": "http",
    ...
    options = ClaudeAgentOptions(
        ...
        system_prompt=(
            ...
            "Use the available MCP tools to fetch financial data and return the raw results."
        ),
    ...
    async with ClaudeSDKClient(options=options) as client:
        await client.query(
            f"Call get_bank_transactions with username='{username}', "
            f"start_date='{start_date}', end_date='{end_date}'. "
            f"Then call get_credit_card_transactions with the same parameters. "
            f"Return both result sets verbatim."
        )
            ...
            if isinstance(message, AssistantMessage):
                for block in message.content:
                    if isinstance(block, TextBlock):
                        logger.debug(f"Data fetch response: {block.text[:200]}")
...
async def _run_orchestrator(
    ...
    2. Performs initial analysis (detects subscriptions)
    3. Defines three specialized sub-agents (research, negotiation, tax)
    4. Invokes sub-agents in parallel via ClaudeAgentOptions
    ...
    # Step 3: Define sub-agents (Haiku for cost optimization — 75% cheaper than Sonnet)
    ...
    # Step 5: Execute orchestrator via Claude Agent SDK with streaming output
